...
function draw() {
  ...
  if (showBackdoorHud) drawControls(level, bass, mid, treble);
  pop();
}
...
function drawFramedCan(can, t, level, bass, mid, treble, spectrum) {
  ...
  const hover = mechanicsEnabled && dist(mxArt, myArt, can.x + can.w / 2, can.y + can.h / 2) < min(can.w, can.h) * 0.48 ? 1 : 0;
  const n = noise(can.seed, mechanicsEnabled ? t * 0.7 : 0.7);
  const jump = mechanicsEnabled ? (noise(can.seed + 1, t * 8) - 0.5 + eventPulse * 0.3) * (2 + bass * 14) * can.w / 140 : 0;
  const shake = mechanicsEnabled ? sin(t * (10 + treble * 44) + can.phase) * (0.6 + treble * 5 + hover * 4) : 0;
  const mx = map(mxArt, 0, ART_W, -1, 1);
  const my = map(myArt, 0, ART_H, -1, 1);
  const rot = mechanicsEnabled ? radians((mx * 1.8 + sin(t + can.phase) * 0.6) * (hover ? 1.8 : 1)) : 0;
  ...
  push();
  translate(can.x + can.w / 2 + jump + shake, can.y + can.h / 2 + my * hover * 5);
  rotate(rot);
...
function drawCan(can, t, level, bass, mid, treble, n, hover, spectrum) {
  ...
  const openAmt = constrain(can.lidOpen + bass * can.openResponsiveness * 0.28 + hover * 0.12, 0, 1);
  ...
  push();
  if (mechanicsEnabled) translate(0, level * sin(t * 20 + can.phase) * 9);
...
function drawMetalSheen(can, x, y, w, h, crushAmt, rustAmt) {
  ...
  for (let i = 0; i < 22; i++) {
    ...
    strokeWeight(1);
    line(px + sin(i) * crushAmt * 4, y + h * 0.12, px - sin(i * 0.7) * crushAmt * 6, y + h * 0.91);
  }
}
...
function drawDents(can, x, y, w, h, crushAmt, damageAmt) {
  ...
  for (let i = 0; i < 3; i++) {
    ...
    line(sx, sy, sx + w * (noise(can.seed + 48, i) - 0.5) * 0.22, sy + h * (noise(can.seed + 50, i) - 0.5) * 0.14);
  }
}
...
function keyPressed() {
  if (key >= "1" && key <= "4") mode = int(key) - 1;
  if (key === "r" || key === "R") {
    seedValue = floor(random(100000));
    buildGrid();
  }
  if (key === " ") {
    if (audioFile && audioFile.isPlaying()) audioFile.pause();
    else if (audioFile) audioFile.loop();
  }
  if (key === "u" || key === "U") audioUpload.elt.click();
  if (key === "h" || key === "H") showBackdoorHud = !showBackdoorHud;
}
